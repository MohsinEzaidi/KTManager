/** @format */

import { useEffect, useState } from "react";
import { axiosClient } from "../../../api/axios";
import user1 from "../../../../public/imgs/user1.jpg";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faArrowRightLong,
  faCheck,
  faEllipsisVertical,
  faPlus,
} from "@fortawesome/free-solid-svg-icons";
import { Link } from "react-router-dom";
import TaskFilters from "./components/TaskFilters";
import CommentsPopUp from "./components/CommentsPopUp";

function Tasks() {
  const [tasks, setTasks] = useState([]);
  const [filteredTasks, setFilteredTasks] = useState([]);
  const [allUsers, setAllUsers] = useState([]);
  const [allNecessities, setAllNecessities] = useState([]);
  const [openEditDropdown, setOpenEditDropdown] = useState(null);
  const [commentShow, setCommentShow] = useState(null);
  const [comment, setComment] = useState("");
  const [taskIndex, setTaskIndex] = useState(null);
  const [showComments, setShowComments] = useState(null);
  const [dateFilter, setDateFilter] = useState("");

  const necessiteNames = [
    { necessite: "l", name: "low", color: "bg-green-500" },
    { necessite: "n", name: "normal", color: "bg-gray-500" },
    { necessite: "h", name: "high", color: "bg-orange-500" },
    { necessite: "e", name: "urgent", color: "bg-red-500" },
  ];
  const allStatuses = [
    "saisie",
    "affecte",
    "developpment",
    "test unitaire",
    "test integration",
    "deploiement",
    "done",
  ];

  const getTasks = async () => {
    try {
      const response =
        localStorage.getItem("USER_FUNCTION") === "admin"
          ? await axiosClient.get(
              `/api/taches/taches-by-projet/${localStorage.getItem(
                "PROJECT_ID"
              )}`
            )
          : await axiosClient.get(
              `/api/taches/get-collaborateur-taches/${localStorage.getItem(
                "COLLABORATEUR_ID"
              )}/${localStorage.getItem("PROJECT_ID")}`
            );

      setTasks(response.data.taches);

      const users = response.data.taches.map((task) => task.user.nom);
      const necessities = response.data.taches.map((task) => task.necessite);
      setAllUsers([...new Set(users)]);
      setAllNecessities([...new Set(necessities)]);
    } catch (error) {
      console.error("Error fetching tasks:", error);
    }
  };

  useEffect(() => {
    getTasks();
  }, []);

  const applyFilters = (filters) => {
    let filtered = tasks;
    if (filters.user) {
      filtered = filtered.filter((task) => task.user.nom === filters.user);
    }
    if (filters.status) {
      filtered = filtered.filter(
        (task) => task.status.etape === filters.status
      );
    }
    if (filters.necessity) {
      filtered = filtered.filter(
        (task) => task.necessite === filters.necessity
      );
    }
    if (filters.date) {
      filtered = filtered.filter(
        (task) => task.dateDebut.split(" ")[0] === filters.date
      );
      console.log(`date filter :${filters.date}`);
    }
    setFilteredTasks(filtered);
  };

  const handleEditDropdown = (index) => {
    setOpenEditDropdown(index === openEditDropdown ? null : index);
  };

  const handleDeleteTask = async (id) => {
    try {
      await axiosClient.delete(`/api/taches/${id}`);
      // Filter out the deleted task from tasks state
      const updatedTasks = tasks.filter((task) => task.idTache !== id);
      setTasks(updatedTasks);
      // Filter out the deleted task from filteredTasks state if exists
      if (filteredTasks.length > 0) {
        const updatedFilteredTasks = filteredTasks.filter(
          (task) => task.idTache !== id
        );
        setFilteredTasks(updatedFilteredTasks);
      }
    } catch (error) {
      console.log("error from delete task", error);
    }
  };
  const handleAddComment = async (id) => {
    try {
      await axiosClient.post("/api/comments", {
        idTache: id,
        comment: comment,
        idCollaborateur: localStorage.getItem("COLLABORATEUR_ID"),
      });
      getTasks();
      setCommentShow(null);
      window.location.reload();
    } catch (error) {
      console.log("error from add comment:", error);
    }
  };
  const adjustIndex = (index) => {
    index -= 1;

    return (((index % 12) + 12) % 12) + 1;
  };
  return (
    <div className='tasks basis-full flex flex-wrap justify-center ml-5 my-10'>
      <div className=' bg-secondary py-2 px-5 mb-5 rounded-lg basis-full flex items-center justify-between'>
        <h1 className='text-2xl max-lg:hidden   text-white font-thin'>
          All tasks
        </h1>
        <TaskFilters
          onFilterChange={applyFilters}
          allUsers={allUsers}
          allStatuses={allStatuses}
          allNecessities={allNecessities}
        />
      </div>
      {console.log("all the tasks u will get ************************", tasks)}
      {console.log(
        "all the tasks u will get ------------------------",
        filteredTasks
      )}
      <div className='task-cards basis-full flex flex-wrap justify-between gap-3 py-5 px-5 bg-gray-200 rounded-lg'>
        {tasks.length > 0 ? (
          (tasks ? tasks : filteredTasks).map((task, i) => (
            <li
              key={i}
              className='task-card basis-[31%] max-lg:basis-[48%] max-sm:basis-full bg-white rounded-md shadow-md p-6 flex flex-col justify-between'
            >
              <div className='developer-edit flex justify-between items-center mb-4'>
                <div className='developer flex justify-end flex-row-reverse items-center gap-3 '>
                  <div className='name flex text-sm items-center gap-2 font-medium'>
                    <span className='first-name capitalize'>
                      {task.user.prenom}
                    </span>
                    <span className='last-name capitalize'>
                      {task.user.nom}
                    </span>
                  </div>
                  <div className='flex-shrink-0'>
                    <img
                      className='w-8 h-8 rounded-full'
                      src={`../../../../public/imgs/user${adjustIndex(
                        task.user.idCollaborateur
                      )}.jpg`}
                      alt='Profile'
                    />
                  </div>
                </div>
                <div className='relative'>
                  <button
                    className='edit text-lg text-gray-500 ml-2 px-1'
                    onClick={() => handleEditDropdown(i)}
                  >
                    <FontAwesomeIcon icon={faEllipsisVertical} />
                  </button>
                  {openEditDropdown === i && (
                    <ul className='absolute top-full right-3 bg-white border border-gray-300 rounded-md mt-1 py-1  divide-y'>
                      {localStorage.getItem("USER_FUNCTION") === "admin" ? (
                        <>
                          <li
                            className='px-3 py-1 cursor-pointer hover:bg-gray-100'
                            // onClick={() => handleEditClick(task.id)}
                          >
                            <Link
                              className='h-full w-full block'
                              to={`${task.idTache}/edit`}
                            >
                              Edit
                            </Link>
                          </li>
                          <li
                            className='px-3 py-1 cursor-pointer hover:bg-gray-100'
                            onClick={() => handleDeleteTask(task.idTache)}
                          >
                            Delete
                          </li>
                          <li
                            className='px-3 py-1 cursor-pointer hover:bg-gray-100 whitespace-nowrap'
                            onClick={() => {
                              setCommentShow(i);
                              handleEditDropdown(null);
                              setTaskIndex(i);
                            }}
                          >
                            Add Comment
                          </li>
                        </>
                      ) : (
                        <li
                          className='px-3 py-1 cursor-pointer hover:bg-gray-100'
                          onClick={() => {
                            setCommentShow(i);
                            handleEditDropdown(null);
                            setTaskIndex(i);
                          }}
                        >
                          Add Comment
                        </li>
                      )}
                    </ul>
                  )}
                </div>
              </div>
              <div className='description text-gray-800'>
                {task.description}
              </div>
              <div className='necessite-date mt-4 flex justify-between items-center text-gray-500'>
                {necessiteNames.map(
                  (nn) =>
                    nn.necessite === task.necessite && (
                      <div
                        key={nn.necessite}
                        className={`necessite ${nn.color} text-white py-1 px-2 rounded-md text-sm`}
                      >
                        {nn.name}
                      </div>
                    )
                )}
                <div className='status'>
                  <span
                    className={` capitalize text-sm ${
                      task.status.etape === "done" &&
                      "text-green-600 font-medium"
                    }`}
                  >
                    {task.status.etape === "done" && (
                      <FontAwesomeIcon className='mr-1' icon={faCheck} />
                    )}
                    {task.status.etape}
                  </span>
                </div>
                <div className='date text-xs flex gap-1 items-center'>
                  <span>{task.formattedDateDebut}</span>
                  {""}
                  <FontAwesomeIcon icon={faArrowRightLong} />{" "}
                  <span className={``}>{task.formattedDateFin}</span>
                </div>
              </div>
              {commentShow !== i ? (
                showComments === i ? (
                  <div className='comments-popup grow'>
                    <CommentsPopUp comments={task.comments} />
                  </div>
                ) : (
                  <div
                    className='grow mt-3 text-gray-500  border-t  pt-5'
                    onClick={() => {
                      setShowComments(i);
                    }}
                  >
                    ({task.comments.length}) Comments
                  </div>
                )
              ) : (
                <div className='comment-form mt-3'>
                  <textarea
                    rows={5}
                    cols={10}
                    className='border-secondary border-2 focus:outline-none w-full py-1 px-2 rounded-md'
                    name='comment'
                    id='comment'
                    placeholder='Add a comment'
                    onChange={(e) => {
                      setComment(e.target.value);
                    }}
                  >
                    {comment}
                  </textarea>
                  <div className='buttons flex gap-2 mt-3'>
                    <button
                      className='py-2 px-3 rounded-lg text-white bg-primary'
                      onClick={() => handleAddComment(task.idTache)}
                    >
                      save
                    </button>
                    <button
                      className='py-2 px-3 rounded-lg text-white  bg-gray-500'
                      onClick={() => {
                        setCommentShow(null);
                      }}
                    >
                      cancel
                    </button>
                  </div>
                </div>
              )}
            </li>
          ))
        ) : (
          <div className='text-center w-full'>
            <div role='status'>
              <svg
                aria-hidden='true'
                className='inline w-8 h-8 text-white animate-spin dark:text-gray-600 fill-blue-600'
                viewBox='0 0 100 101'
                fill='none'
                xmlns='http://www.w3.org/2000/svg'
              >
                <path
                  d='M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z'
                  fill='currentColor'
                />
                <path
                  d='M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z'
                  fill='currentFill'
                />
              </svg>
              <span className='sr-only'>Loading...</span>
            </div>
          </div>
        )}
        {localStorage.getItem("USER_FUNCTION") === "admin" &&
        tasks.length > 0 ? (
          <Link
            to={"add-task"}
            className='task-card basis-[31%] bg-white rounded-md shadow-md p-6 flex flex-col justify-between'
            onClick={() => {}}
          >
            <div className='flex items-center justify-center h-full'>
              <FontAwesomeIcon
                icon={faPlus}
                className='text-4xl text-gray-500 '
              />
            </div>
          </Link>
        ) : (
          <></>
        )}
      </div>
    </div>
  );
}

export default Tasks;
